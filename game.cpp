...
char    *FILE_GAMESKIN = "data/gameskin.tga";
...
{
	printf("GameInitialSetup()\n" );
	// Set up the in-game menu
	gameMenu.m_clrBack = djColor(0,0,129);
	gameMenu.m_iSize = 0;
	gameMenu.m_items = gameMenuItems;
	gameMenu.m_szCursor = gameMenuCursor;
	gameMenu.m_xOffset = -1; // Calculate menu position for us
	gameMenu.m_yOffset = -1;

	printf("GameInitialSetup(): loading sounds\n" );
	...
	// If main game skin not yet loaded, load it
	if (pSkinGame==NULL)
	{
		if (NULL != (pSkinGame = new djImage))
			pSkinGame->Load( FILE_GAMESKIN );
		else
			djMSG( "GameDrawSkin(): error loading skin image file.\n" );
	}
}
...
void PerLevelSetup()
{
	...
	char * szfilename = g_pCurMission->GetLevel( g_nLevel )->GetFilename( );

	// always keep current level loaded at slot 0
	TRACE("PerLevelSetup(): level_load( %s )\n", szfilename );
