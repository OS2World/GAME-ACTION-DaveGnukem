...
static void MoveMinimap( int ox, int oy, bool bredrawview = true );
...
static void SetLevel( int x, int y, int a, int b, bool bforeground );
...
switch_e LVLED_MainLoop ()
{
	...
	while ( bRunning )
	{
		...
		{
			bflagleft = true;
			MoveMinimap( levelview_x - 1, levelview_y, false );
			SDL_Delay( delay );
		}
		...
		{
			bflagup = true;
			MoveMinimap( levelview_x, levelview_y - 1, false );
			SDL_Delay( delay );
		}
...
TODO: remove most of the crap from here. LVLED will work in
realtime anyway, so no need to bother with this clear-redraw
stuff.
=================
*/
static void MoveMinimap( int ox, int oy, bool bredrawview )
{
